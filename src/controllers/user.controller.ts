...
import { LoginDto, SignUpDto } from "../common/schema/user.schema";
...
const signupUser = async (req: Request, res: Response, next: NextFunction) => {
    const { firstName, lastName, emailId, password, userType } =
...
const loginUser = async (req: Request, res: Response, next: NextFunction) => {
    const { emailId, password } = req.body as LoginDto;
    // validate that the required fields are present in the request body
    ...
    try {
        const existingUser = await User.findOne({ emailId });
        if (!existingUser) {
            const invalidCredentials = createHttpError(
                404,
                "Invalid credentials.",
            );
        ...
        const isProvidedPasswordCorrect =
        ...
        return next(
            createHttpError(500, "Something went wrong while logging in."),
        );
    }
};
...
const refreshToken = async (
    ...
    try {
        ...
        if (typeof decodedToken === "string" || !decodedToken?._id) {
            const invalidTokenError = createHttpError(
                401,
                "Invalid credentials.",
            );
        ...
        if (user.refreshToken !== incomingToken) {
            return next(
                createHttpError(403, "Invalid or expired refresh token."),
            );
        }
